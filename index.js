...
const _GETMAX = async(t) => {
		return new Promise(function(resolve, reject) {
				...
				let max = __.last(__.map(__.filter(FS.readdirSync(CONFIG.geomdir), (f) => {
					return f.split(".")[0] == typ
				}), (b) => {
...
const _LOGTRACK = async(K) => {
		return new Promise(function(resolve, reject) {
				...
				FS.exists(fn, (exists) => {
					...
					}) : FS.writeFileSync(fn, JSON.stringify(K));
					resolve({
						success: true,
						msg: `${K.features.length} saved`
					})
				});
...
const _sendP = async(D) => {
	...
	/*const _update_missing = async (cid,ctyp) => {
...
APP.get('/ebl/tracks', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");

	let tracks = __.compact(__.map(FS.readdirSync(CONFIG.ebltrackdir), t => {
		// console.log("t", t);
		return t.indexOf('geojson') >= 0 ? JSON.parse(FS.readFileSync(`${CONFIG.ebltrackdir}/${t}`)) : null;
	}))

	res.send({
		success: true,
		msg: `${tracks.length} tracks found`,
		payload: tracks
	})
});

APP.get('/ebl/ors/:outback', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");

	res.send(JSON.parse(FS.readFileSync(`./static/ors-sample-${req.params.outback}.geojson`)))


});
APP.get('/ebl/brookline-outline', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");

	// res.send(JSON.parse(FS.readFileSync(`./static/brookline.geojson`)))
	res.send(JSON.parse(FS.readFileSync(`./static/brookline-outline.geojson`)))


});
APP.get('/ebl/brookline', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");

	// res.send(JSON.parse(FS.readFileSync(`./static/brookline.geojson`)))
	res.send(JSON.parse(FS.readFileSync(`./static/brookline-outline-buffer.geojson`)))


});
APP.get('/ebl/here', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");

	// res.send(JSON.parse(FS.readFileSync(`./static/brookline.geojson`)))
	res.send(JSON.parse(FS.readFileSync(`./static/here.routes-sample.json`)))


});
APP.get('/ebl/brooklinesghost', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");
...
APP.get('/ebl/centerlines', async(req, res) => {
	res.header("Access-Control-Allow-Origin", "*");
	...
	res.send({
		success: true,
		msg: `brookline OSM centerlines`,
		payload: JSON.parse(FS.readFileSync(`./static/brookline-osm-centerlines.geojson`))
	})
});
...
APP.get('/about', (req, res) => {
	var F = {
		name: "Milleria API",
		"description": "This is a private API, largely, that supports much of the work at *.milleria.org"
	}
			...
			MONGO.connect(url, (err, client) => {
				...
				col.find(query).limit(999999).toArray((err, docs) => {
					if (err) {
						...
						let resp = {
							success: false,
							"msg": "conflicting id/geom coupling for " + qt + ":" + qv
						}
